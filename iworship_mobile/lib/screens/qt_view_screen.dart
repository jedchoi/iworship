...
class _QtViewScreenState extends State<QtViewScreen> with SingleTickerProviderStateMixin {
  ...
  void _syncControllersWithProvider(QtProvider provider) {
    ...
    if (note != null) {
      if (_sermonTitleCtrl.text != note.todayThanks) _sermonTitleCtrl.text = note.todayThanks;
      if (_thanksCtrl.text != note.todayThanks) _thanksCtrl.text = note.todayThanks;
      if (_engravedCtrl.text != note.engravedWord) _engravedCtrl.text = note.engravedWord;
      if (_appCtrl.text != note.todayApplication) _appCtrl.text = note.todayApplication;
      if (_prayerCtrl.text != note.todayPrayer) _prayerCtrl.text = note.todayPrayer;
      if (_ibs1Ctrl.text != note.sundayAnswer1) _ibs1Ctrl.text = note.sundayAnswer1;
      if (_ibs2Ctrl.text != note.sundayAnswer2) _ibs2Ctrl.text = note.sundayAnswer2;
      if (_ibs3Ctrl.text != note.sundayAnswer3) _ibs3Ctrl.text = note.sundayAnswer3;
      if (_sermonCtrl.text != note.sermonNote) _sermonCtrl.text = note.sermonNote;
    } else {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // Verses List with In-line Selection Highlight & Note Box
              ...
              children: [
                ...
                InkWell(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      color: isVerseSelected ? const Color(0xFFF3EDF7) : Colors.transparent,
                      borderRadius: BorderRadius.circular(8),
                      border: isVerseSelected ? Border.all(color: const Color(0xFF8E7CA2), width: 1.5) : null,
                    ),
                    child: Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                ...
                // Inline Verse Memo Box (Appears directly below selected verse)
                if (isVerseSelected) ...[
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        TextField(
                          controller: _verseMemoCtrls[v.num],
                          maxLines: 2,
      ...
      child: Column(
        ...
        children: [
          if (provider.isSunday) ...[
            ...
            _buildInputCard('1. 오늘 말씀을 통해 알게 된 하나님은 어떠한 하나님인가요?', _ibs1Ctrl),
            _buildInputCard('2. 오늘 말씀을 통해 깨달은 것과 받은 은혜는 무엇인가요?', _ibs2Ctrl),
            _buildInputCard('3. 나는 한 주간 어떤 삶을 살 것인지 다짐하고 함께 나누어 봅시다.', _ibs3Ctrl),
            const SizedBox(height: 20),
            _buildSectionHeader('✍️ 주일 설교 NOTE (설교 제목 & 메시지 메모)'),
            _buildInputCard('설교 제목 및 본문 요약 메모를 자유롭게 적어보세요', _sermonCtrl, maxLines: 8),
          ] else ...[
            // === WEEKDAY FORM ===
            ...
            _buildInputCard('1. 오늘의 감사', _thanksCtrl),
            _buildInputCard('2. 아로새길 말씀 (1페이지 구절 터치 시 자동 추가 가능)', _engravedCtrl),
            _buildInputCard('3. 오늘의 적용', _appCtrl),
            _buildInputCard('4. 오늘의 기도 (주신 말씀으로 살아낼 은혜를 구하는 기도)', _prayerCtrl),
          ],
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: const Text('묵상 기록 저장하기 (서버 백업 동기화)', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              style: ElevatedButton.styleFrom(
                backgroundColor: const Color(0xFF7C6893),
                foregroundColor: Colors.white,
                padding: const EdgeInsets.all(16),
              ...
              onPressed: () async {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('🎉 묵상 기록이 로컬 SQLite에 저장되고 서버로 백업 동기화되었습니다!')),
                );
              },
            ),
          ),
  ...
  Widget _buildInputCard(String label, TextEditingController controller, {int maxLines = 3}) {
    return Container(
